...
import { useUpdateSection } from '../../../../hooks/manager/sections/useSectionMutation';
import { createPortal } from 'react-dom';
// import { useUpdateSection } from '../../../../hooks/manager/sections/useSections';
...
function EditSectionModal({ isOpen, onClose, section }: EditSectionModalProps) {
    ...
    useEffect(() => {
        if (isOpen && section) {
            ...
            setLocalCounselorId(section.counselorId || null);
        }
    }, [isOpen, section]);

    if (!isOpen) return null;

    const counselors = counselorsData?.data.counselors.filter(c => c.localEmployeeNumber !== section.counselorId) || [];
    ...
    const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
        ...
        try {
            await updateSection({
                sectionId: section.localSectionNumber,
                gradeId: section.localGradeNumber,
                data: {
                    name: name.trim(),
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-3">
                        <div className="bg-dark-blue-100 rounded-full p-2.5">
                            <FaSave className="text-dark-blue-700 text-xl" />
                        </div>
                        <h3 className="text-xl font-bold text-dark-blue-800">Edit Section</h3>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="mb-5 p-4 bg-blue-gray-50 rounded-xl">
                    <p className="text-xs text-blue-gray-500 uppercase tracking-wider font-medium">Section</p>
                    <p className="text-lg font-semibold text-dark-blue-800">{section.name}</p>
